...
public class RedirectionManager : MonoBehaviour {
    ...
    //public float realWorldRotationGain = 0.5f; // 增益系数，可以根据需要调整
    ...
    void UpdateBodyPose()
    {
        ...
        //Quaternion hmdRotation = InputTracking.GetLocalRotation(XRNode.CenterEye); // 将头显的旋转应用到物体上，同时应用增益
        ...
        body.rotation = Quaternion.LookRotation(Utilities.FlattenedDir3D(headTransform.forward), Vector3.up);//Quaternion.LookRotation()函数用于计算从一个向量指向另一个向量所需要的旋转角度，并返回一个四元数表示该旋转
    }
